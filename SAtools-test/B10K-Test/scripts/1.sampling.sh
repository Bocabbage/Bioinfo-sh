...
do
    $seqtk sample -s${SEED} $DB/$FA/${FA}.fa $DQNUMS > FASTA/${FA}_query.fa.temp && \
...
$python $WORKPATH/scripts/FixLenQueries.py -i FASTA/query.fa -o FASTA/query-seed${SEED}_fixlen-${QLEN}.fa --length $QLEN --seed ${SEED}
